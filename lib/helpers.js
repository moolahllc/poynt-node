...
module.exports.nakedRequest = function nakedRequest(options, next) {
  ...
  if (options.form) {
    ...
    fetchOptions.body = querystring.stringify(options.body);
  ...
  debug("Poynt API request:", url, fetchOptions.method, fetchOptions.body);
  ...
  fetch(url, fetchOptions)
    .then(function (res) {
      ...
      return res.text().then(function (text) {
        ...
        debug("Poynt API response:", statusCode, JSON.stringify(body));

        if (statusCode < 300) {
          return next(null, body);
        }

        var requestErr = (typeof body === "object" && body) ? body : {};
        requestErr.requestId = requestId;
        if (!requestErr.httpStatus) {
          requestErr.httpStatus = statusCode;
        }
    ...
    .catch(function (err) {
      debug("Poynt API error:", err.message);
      next(module.exports.error({
