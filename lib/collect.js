 ...
 * @param {Object} options.amounts – amounts object containing currency, transactionAmount, orderAmount, cashbackAmount, tipAmount, transactionFees (optional)
...
module.exports.chargeToken = function (options, next) {
  ...
  var amounts = {
    ...
    customerOptedNoTip: options.amounts.customerOptedNoTip || false,
    tipAmount: options.amounts.tipAmount || 0,
    transactionAmount: options.amounts.transactionAmount
  };

if (options.amounts.transactionFees && options.amounts.transactionFees.amount)
 {
    amounts.transactionAmount += Number(options.amounts.transactionFees.amount);
    amounts.transactionFees = [{
      amount: Number(options.amounts.transactionFees.amount) || 0,
      feeType: options.amounts.transactionFees.feeType || null,
      signature: options.amounts.transactionFees.signature || null,
      required: options.amounts.transactionFees.required || false,
      feeProgramType: options.amounts.transactionFees.feeProgramType || null,
      feeProgramId: options.amounts.transactionFees.feeProgramId || null
    }]
  };
  ...
  if (options.customerPresenceStatus) {
    body.fundingSource.entryDetails.customerPresenceStatus =
    options.customerPresenceStatus || "";
  }

  if (options.entryMode) {
    body.fundingSource.entryDetails.entryMode = options.entryMode || "";
  }
  ...
  if (options.sourceApp) {
    body.context.sourceApp = options.sourceApp || "Bridge";
  }
  if (options.source) {
  ...
  if (options.TransactionNumber) {
    body.TransactionNumber = options.TransactionNumber || "";
  }
